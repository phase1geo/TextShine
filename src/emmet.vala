...
public class emmet {
  ...
  private Regex _prespace_re;
  private Regex _tag_re;
  private Regex _brack_sp_re;
  private Regex _tagname_re;
  ...
  */
  ...
  public emmet() {
    try {
      _prespace_re = new Regex( "^([ \\t]*)" );
      _tag_re      = new Regex( "(.*)(<\\/?[\\w:-]+(?:\\s+[\\w:-]+(?:\\s*=\\s*(?:(?:".*?")|(?:'.*?')|[^>\\s]+))?)*\\s*(\\/?)>)" );
      _brack_sp_re = new Regex( "(.*?)(\\[.*?\\]|\\{.*?\\}|\\s+)" );
      _tagname_re  = new Regex( "[a-zA-Z0-9_:-]+" );
    } catch( Regex.Error e ) {}
  }
  ...
  */
  public void get_snippet_text_pos( string input, int cursor, out string str, out int start, out string prespace ) {
    ...
    var line_start = input.last_index_of( "\n", cursor );
    if( last_start == -1 ) {
      line_start = 0;
    } else {
      unichar c;
      line_start = last_newline;
      if( input.get_next_char( ref line_start, out c ) ) {
        str = input.splice( line_start, cursor );
      }
    }
    ...
    if( _prespace_re.match( str, 0, out match_info ) {
      prespace = match_info.fetch( 0 );
    } else {
      prespace = "";
    }
    ...
    var start_index = 0;
    if( _tag_re.match( str, 0, out match_info ) ) {
      start_index = match_info.fetch( 0 ).length;
    }

    /* Find a space that does not exist within a bracket */
    try {
      var col = start_index;
      while( _brack_sp_re.match_full( str, -1, col, 0, match_info ) ) {
        var match = match_info.fetch( 1 );
        if( (match == " ") || (match == "\t") ) {
          last_space_pos = match.length;
        }
        col = pos.index( pos.length - 1 );
      }
    }

    if( last_space_pos == -1 ) {
      str   = input.slice( input.index_of_nth_char( start_index ), input.index_of_nth_char( cursor ) );
      start = start_index;
    } else {
      str   = input.slice( input.index_of_nth_char( last_space_pos ), input.index_of_nth_char( cursor ) );
  ...
  */
  public string expand_abbreviation( string input, int cursor ) {
    ...
    if {$lang eq "CSS"} {
      ...
      if {[regexp {(\S+)$} [$txt get "insert linestart" insert] -> abbr]} {
        if {![catch { mmet_css::parse $abbr } str]} {
    ...
    */
    ...
    get_snippet_text_pos( input, cursor, out string str, out int startpos, out int endpos, out string prespace );

    /* Parse the snippet and if no error, return new string */
    try {
      var new_str = parse_emmet( str, prespace );
      return( input.splice( startpos, endpos, new_str ) );
    } catch( Error e ) {}
