...
public class ConvertURL : TextFunction {
  ...
  public class URLEntry {
    ...
    public string win;
    public string utf8;
    public URLEntry( string ch, string win, string utf8 ) {
      this.ch   = ch;
      this.win  = win;
      this.utf8 = utf8;
    }
    public string encode( string str, bool use_utf8 = true ) {
      return( str.replace( ch, (use_utf8 ? utf8 : win) ) );
    }
    public string decode( string str, bool use_utf8 = true ) {
  ...
  public class URLEntries {
    ...
    public void add( string ch, string win, string utf8 ) {
      var entry = new URLEntry( ch, win, utf8 );
      _entries.append_val( entry );
    }
    public string encode( string str, bool use_utf8 = true ) {
      var estr = str;
      for( int i=0; i<_entries.length; i++ ) {
        estr = _entries.index( i ).encode( estr, use_utf8 );
      }
      return( estr );
    }
    public string decode( string str, bool use_utf8 = true ) {
  ...
  private void populate_table() {
    ...
    _entries.add( "™",  "%99", "%E2%84" );
    _entries.add( "š",  "%9A", "%C5%A1" );
    _entries.add( "›",  "%9B", "%E2%80" );
  ...
  public override string transform_text( string original, int cursor_pos ) {
    if( direction == FunctionDirection.LEFT_TO_RIGHT ) {
      return( _entries.encode( original, _type.use_utf8() ) );
    } else {
